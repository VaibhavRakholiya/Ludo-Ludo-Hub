...
public class PlayFabManager : Photon.PunBehaviour, IChatClientListener
{
    ...
    public void FailedToFindPlayers()
    {
        ...
        //     GameManager.Instance.dialogFails.SetActive(true);
        // }
        GameManager.Instance.dialogFails.SetActive(true);
        ...
        // showMenu();
        // GameManager.Instance.controlAvatars.ShowJoinFailed("Players Not Available");
    }
    ...
    {
        y = 0;
        dexes = new int[] { 100, 100, 100, 100, 100, 100 };
        e = 0;
        dexe = new int[] { 100, 100, 100, 100, 100, 100 };
        ...
        if (PhotonNetwork.isMasterClient)
        {
            Debug.Log("hellllll=======>" + PlayerPrefs.GetInt("botEnebal"));
            if (PlayerPrefs.GetInt("botEnebal") == 1)
            {
                Invoke("AddBots", 3.0f);
            }
            else
    ...
    int y = 0;
    private int[] dexes = { 100, 100, 100, 100, 100, 100 };
    int e = 0;
    private int[] dexe = { 100, 100, 100, 100, 100, 100 };
    public IEnumerator AddBot(int i)
    {
        yield return new WaitForSeconds(i + UnityEngine.Random.Range(0.0f, 0.9f));
        { 
    newIndex:
        var MyIndex = UnityEngine.Random.Range(0, avatarSprites.Length);
        ...
        {
        newInd:
            var MyIndex = UnityEngine.Random.Range(0, GameManager.Instance.botsName.Length);
    ...
    public void addCoinsRequest(int count)
    {
        ...
        Debug.Log("AddCoinRequest is check : " + data[MyPlayerData.CoinsKey]);
        string url = StaticStrings.baseURL + "update-user-coins.php?playfab_id=" + GameManager.Instance.playfabManager.PlayFabId + "&avl_coins=" + data[MyPlayerData.CoinsKey];
        WWW www = new WWW(url);
    ...
    private IEnumerator getVerificationStatus(WWW www)
    {
        yield return www;
        ...
        if (www.error == null)
        {
            if (www.text == "1")
            {
                Debug.Log("User verified : " + www.text);
                StartCoroutine(loadSceneMenu());
            }
            else if (www.text == "0")
            {
                GameManager.Instance.facebookManager.showOtpDialog();
                Debug.Log("User is not verified : " + www.text);
            }
            else
            {
                GameManager.Instance.facebookManager.serverStatusDialog.SetActive(true);
                serverStatusInformationText.GetComponent<Text>().text = "Your account is not active, kindly contact with support!";
                //  GameManager.Instance.dialogNew.dialogTxt.GetComponent<Text>().text = www.text;
            }
        }
        else
        {
            GameManager.Instance.facebookManager.serverStatusDialog.SetActive(true);
            serverStatusInformationText.GetComponent<Text>().text = www.text;
    ...
    {
        // GameManager.Instance.facebookManager.showResendotpbtn();
        otpSubmitInformationText.SetActive(false);
        if (otpInputField.text == PlayerPrefs.GetString("otp_code"))
        {
            Debug.Log("OTP Verified Successfully : " + otpInputField.text + " Otp Code is : " + PlayerPrefs.GetString("otp_code"));
            string url = StaticStrings.baseURL + "update-user-status.php?playfab_id=" + GameManager.Instance.playfabManager.PlayFabId;
            WWW www = new WWW(url);
            StartCoroutine(updateUserStatus(www));

            // GameManager.Instance.facebookManager.closeOtpDialog();
            // StartCoroutine(loadSceneMenu());
        }
        else
        {
            otpSubmitInformationText.SetActive(true);
            otpSubmitInformationText.GetComponent<Text>().text = "Incorrect Otp! Verification Failed. ";
        }


    }
    private IEnumerator updateUserStatus(WWW www)
    {
        yield return www;
        ...
        if (www.error == null)
        {
            ...
            {
                otpSubmitInformationText.SetActive(true);
                otpSubmitInformationText.GetComponent<Text>().text = "Status Update failed. ";
                //GameManager.Instance.facebookManager.showOtpDialog();
                Debug.Log("Unable to update user status : " + www.text);
            }
    ...
    {
        if (regiterEmail.GetComponent<Text>().text == "" || registerUsername.GetComponent<Text>().text == "" || InputField2.text == "" || registerNickname.GetComponent<Text>().text == "")
        {
        ...
        else
        {
            ...
            string invite = registerInviteCode.GetComponent<Text>().text;
            string username = registerUsername.GetComponent<Text>().text;
            string whatsappNumber = registerWhatsappNumber.GetComponent<Text>().text;
            Debug.Log("Email is : " + email + " Password is : " + password + " Username is : " + username);
            ...
            if (Regex.IsMatch(email, @"^(?("")("".+?(?<!\\)""@)|(([0-9a-z]((\.(?!\.))|[-!#\$%&'\*\+/=\?\^`\{\}\|~\w])*)(?<=[0-9a-z])@))" +
                @"(?(\[)(\[(\d{1,3}\.){3}\d{1,3}\])|(([0-9a-z][-\w]*[0-9a-z]*\.)+[a-z0-9][\-a-z0-9]{0,22}[a-z0-9]))$") && nickname.Length > 0 && username.Length >= 10)
            {
    ...
    IEnumerator createInitUser(WWW www)
    {
        ...
        if (www.error == null)
        {
            ...
            results = GetDataValue(msg, "value:");
            status = GetDataValue(msg, "message:");
            coin = GetDataValue(msg, "coin:");
            Debug.Log("AA" + coin);
            Debug.Log("bb" + results);
            GameManager.Instance.coinsCount = int.Parse(coin);
    ...
    //     // form.AddField("player_name", GameManager.Instance.nameMy);
    ...
    public void LinkFacebookAccount()
    {
        ...
        PlayFabClientAPI.LinkFacebookAccount(request, (result) =>
        {
            ...
            string url = StaticStrings.baseURL+"update-users.php?playfab_id="+GameManager.Instance.playfabManager.PlayFabId+"&avl_coins="+data[MyPlayerData.CoinsKey]+"&logged_type="+loggedType+"&facebook_id="+facebookId;
            WWW www = new WWW(url);
    ...
    public void LoginWithFacebook()
    {
        ...
        PlayFabClientAPI.LoginWithFacebook(request, (result) =>
        {
            ...
            if (result.NewlyCreated){
                ...
                // email="+emailId+"&password="+PlayerPrefs.GetString("password")+"&playfab_id="+PlayFabId+"&player_name="+GameManager.Instance.nameMy+"&logged_type="+PlayerPrefs.GetString("LoggedType")
    ...
    {
        string referralUrl = StaticStrings.baseURL + "get-referral-code.php?playfab_id=" + GameManager.Instance.playfabManager.PlayFabId;
    ...
    public void LoginWithEmailAccount()
    {
        ...
        string username = "";
        string password = "";
        // Debug.Log("Password Input Field Is : ", InputField.text);
        if (PlayerPrefs.HasKey("email_account") || PlayerPrefs.HasKey("phone_number"))
        {
            //login with username added
            username = PlayerPrefs.GetString("phone_number");
            email = PlayerPrefs.GetString("email_account");
            password = PlayerPrefs.GetString("password");
        }
        else
        {
            //login with username added and email disabled
            // email = loginEmail.GetComponent<Text>().text;
            username = loginEmail.GetComponent<Text>().text;
    ...
    */
    ...
    private IEnumerator getServerStatus(WWW www)
    {
        yield return www;
        ...
        if (www.error == null)
        {
            ...
            ServerDetail D = JsonUtility.FromJson<ServerDetail>(data2);

            for (int i = 0; i < D.data.Count; i++)
            {
                Debug.Log("Server details: " + D.data[i].status + "app version : " + D.data[i].app_version);// contains all the data sent from the server
                PlayerPrefs.SetString("server_status", D.data[i].status);
                PlayerPrefs.SetString("api_key", D.data[i].api_key);
                PlayerPrefs.SetString("app_version", D.data[i].app_version);
                PlayerPrefs.Save();
            }

            if (PlayerPrefs.GetString("server_status") == "1")
            {

                //             PlayFabClientAPI.GetPhotonAuthenticationToken(request, OnPhotonAuthenticationSuccess, OnPlayFabError);
                Debug.Log("Server is running : " + www.text);
            }
            else
            {
                Debug.Log("App Version Changed: " + www.text);// contains all the data sent from the server
                GameManager.Instance.facebookManager.serverStatusDialog.SetActive(true);
                serverStatusInformationText.GetComponent<Text>().text = "An update is available, please update to continue!";
            }
        ...
        {
            Debug.Log("Server is in maintainence: " + www.text);// contains all the data sent from the server
            GameManager.Instance.facebookManager.serverStatusDialog.SetActive(true);
            serverStatusInformationText.GetComponent<Text>().text = "Hi! Our server is in maintainence, will get back soon.";
            //  GameManager.Instance.dialogNew.dialogTxt.GetComponent<Text>().text = www.text;
        }
    ...
    public override void OnJoinedRoom()
    {
        ...
        if (!roomOwner)
        {
            ...
            for (int i = 0; i < PhotonNetwork.otherPlayers.Length; i++)
            {
                ...
                string name = otherID.Split('|')[0].ToString();
                string otherId = otherID.Split('|')[1].ToString();
    ...
    public override void OnPhotonPlayerConnected(PhotonPlayer newPlayer)
    {
        ...
        */
        ...
        getOpponentData(newPlayer.NickName.Split('|')[0].ToString(), newPlayer.NickName.Split('|')[1].ToString(), index);
    ...
    public IEnumerator loadImageOpponent(string url, int index, string id)
    {
        ...
        using (UnityWebRequest www = UnityWebRequestTexture.GetTexture(url))
        {
         ...
         StartCoroutine(       loadImageOpponent("https://xp.io/storage/11gJNQrl.png", index,id));
            }
            else
            {
