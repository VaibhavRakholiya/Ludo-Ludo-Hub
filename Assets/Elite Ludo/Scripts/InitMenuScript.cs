    ...
    public IEnumerator Details()
    {
        ...
        using (UnityWebRequest handshake = UnityWebRequest.Post(url, form))
        {
            ...
            else 
            {
                ...
                {
                    int playersTwo = UnityEngine.Random.Range(0, 9);
                    TwoplayerOnline.text = "Online Player - 2" + playersTwo;
                    int playersfour = UnityEngine.Random.Range(0, 9);
                    FourplayerOnline.text = "Online Player - " + playersfour;
                   ...
                   //     int TotalCoins = jsonNode["playerdata"]["playcoin"].Value == null ? int.Parse(jsonNode["playerdata"]["totalcoin"].Value.ToString())
                    ...
                    int botEnebal = int.Parse(jsonNode["gameconfig"]["bot_status"].Value);
                    PlayerPrefs.SetInt("botEnebal", botEnebal);
